...
<<<<<<< HEAD
3. **Vault Integration on `/wallets` Tab**: The "Deposit into Vault" dummy SOL transfer mock has been completely refactored. The `/wallets` page now directly accesses `@solana/web3.js` and `@solana/spl-token` natively, mapping the exact `34sgN4q5CaaGCwqePU6d2y6xzBuY5ASA8E8LtXjfyN3c` Treasury Vault Program Programmable Transaction Block (PTB) layout.
=======
3. **Vault Integration on `/wallets` Tab**: The "Deposit into Vault" dummy SOL transfer mock has been completely refactored. The `/wallets` page now directly accesses `@solana/web3.js` and `@solana/spl-token` natively, mapping the exact `FSRUKKMxfWNDiVKKVyxiaaweZR8HZEMnsyHmb8caPjAy` Treasury Vault Program Programmable Transaction Block (PTB) layout.
