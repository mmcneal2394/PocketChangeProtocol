...
const KMS_BACKED_KEYS = (process.env.WALLET_SECRET_KEYS_B58 || '')
    .split(',')
    .map((key) => key.trim())
    .filter(Boolean);
...
async function bootEngine() {
    ...
    if (KMS_BACKED_KEYS.length === 0) {
        console.log('[CORE] No WALLET_SECRET_KEYS_B58 values provided. Exiting without activating live engine.');
        return;
    }
