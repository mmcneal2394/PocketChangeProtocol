...
export async function autoRefillWsol(
  ...
  if (wsolBal >= minWsolSol) return; // Only refill when truly low

  const nativeLam = await connection.getBalance(wallet.publicKey);
  const nativeSol = nativeLam / LAMPORTS_PER_SOL;
  const maxWrap = nativeSol - MIN_NATIVE_SOL_RESERVE;
  // Wrap up to 0.5 SOL at a time to avoid excessive wraps
  const wrapAmount = Math.min(0.5, Math.max(0, maxWrap));
  if (wrapAmount < 0.005) {
    console.log(`[WSOL] ⚠️ Not enough native SOL to refill (${nativeSol.toFixed(4)} - ${MIN_NATIVE_SOL_RESERVE} reserve)`);
    return;
  }
  console.log(`[WSOL] 🔄 Auto-refill: wrapping ${wrapAmount.toFixed(4)} SOL → WSOL (bal was ${wsolBal.toFixed(4)})`);
  await wrapSol(connection, wallet, wrapAmount);
}
