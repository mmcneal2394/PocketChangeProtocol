...
exports.globalArbEngine = exports.ArbEngine = exports.globalAdaptiveMemory = void 0;
const price_book_1 = require("./price_book");
...
class AdaptiveMemory {
    ...
    recordOutcome(routeKey, profitBps) {
...
class ArbEngine {
    ...
    calculateDynamicNetProfit(grossProfitSol, amountInSol) {
        ...
        let jitoTipLamports = expectedProfitLamports * (config_1.config.TIP_PERCENTAGE || 0.5);
        ...
        const slippageTolerance = (config_1.config.MAX_SLIPPAGE_BPS || 50) / 10000;
            ...
            for (let j = 0; j < pools.length; j++) {
                ...
                for (let k = 0; k < pools.length; k++) {
                    ...
                    if (poolA.tokenA === "SOL" && poolC.tokenB === "USDC") {
                        let inter1 = price_book_1.globalPriceBook.calculateOutput(poolA, startingSol, true);
                        let inter2 = price_book_1.globalPriceBook.calculateOutput(poolB, inter1, true); // Assuming Token A -> Token B natively maps
                        let finalOut = price_book_1.globalPriceBook.calculateOutput(poolC, inter2, false);
            ...
            for (let j = i + 1; j < pools.length; j++) {
                for (let k = j + 1; k < pools.length; k++) {
                    ...
                    if (poolA.tokenA === poolB.tokenA && poolB.tokenA === poolC.tokenA) {
                        let intermediateTokens = price_book_1.globalPriceBook.calculateOutput(poolA, startingSol, true);
                        let splitOut1 = price_book_1.globalPriceBook.calculateOutput(poolB, intermediateTokens * 0.5, false);
                        let splitOut2 = price_book_1.globalPriceBook.calculateOutput(poolC, intermediateTokens * 0.5, false);
                        let totalOut = splitOut1 + splitOut2;
                        let gross = totalOut - startingSol;
                        if (gross > config_1.config.MIN_PROFIT_SOL) {
                            const { netProfit, jitoTip } = this.calculateDynamicNetProfit(gross, startingSol);
                            if (netProfit > config_1.config.MIN_PROFIT_SOL) {
                                opportunities.push({
                                    type: 'Split-Routing',
        ...
        for (let i = 0; i < pools.length; i++) {
            for (let j = i + 1; j < pools.length; j++) {
                ...
                if (poolA.tokenA === poolB.tokenA && poolA.tokenB === poolB.tokenB) {
                    ...
                    let intermediate = price_book_1.globalPriceBook.calculateOutput(poolA, startingSol, true);
                    let finalOut1 = price_book_1.globalPriceBook.calculateOutput(poolB, intermediate, false);
                    let gross1 = finalOut1 - startingSol;
                    if (gross1 > config_1.config.MIN_PROFIT_SOL) {
                    ...
                    let intermediate2 = price_book_1.globalPriceBook.calculateOutput(poolB, startingSol, true);
                    let finalOut2 = price_book_1.globalPriceBook.calculateOutput(poolA, intermediate2, false);
                    let gross2 = finalOut2 - startingSol;
                    if (gross2 > config_1.config.MIN_PROFIT_SOL) {
    ...
    async runArbitrageScan() {
        ...
        const pools = price_book_1.globalPriceBook.getAllPools();
        if (pools.length < 2)
            return; // Need at least two pools for arb
        ...
        if (Math.random() < 0.01) {
            logger_1.logger.info(`[SCAN] Checking ${pools.length} pools internally mapped...`);
        }
        if (validOpps.length > 0) {
            ...
            exports.globalAdaptiveMemory.recordOutcome(routeKey, spreadBps);
            logger_1.logger.info(`[FOUND] SOL/USDC: ${routeKey} (${spread.toFixed(1)}% spread | EMA: ${(exports.globalAdaptiveMemory.getExpectedProfit(routeKey) / 100).toFixed(2)}%)`);
            logger_1.logger.info(`[PROFIT] Gross: ${best.grossProfitSol.toFixed(4)} SOL | Tip: ${best.tipAmount.toFixed(4)} SOL | Net: ${best.netProfit.toFixed(4)} SOL ✅`);
            logger_1.logger.info(`[PERF] Detection latency: ${calcTime.toFixed(2)}ms\n`);
            ...
            price_book_1.globalPriceBook.getAllPools().forEach(p => p.reserveB = 15000n);
            // Native execution without verification wrappers
            await this.executeArbitrage(best);
        }
    }
    async executeArbitrage(opp) {
        if (Date.now() - this.lastExecutionTs < 5000)
            return; // Sandbox 5s Throttle Rate Limit
        this.lastExecutionTs = Date.now();
        try {
            logger_1.logger.info(`[COMPILATION] Constructing payload for ${opp.type} with tip: ${opp.tipAmount} SOL`);
            const API_KEY = config_1.config.JUPITER_API_KEY || '05aa94b2-05d5-4993-acfe-30e18dc35ff1';
            const fetchWithTimeout = async (url, options, retries = 3) => {
                for (let i = 0; i < retries; i++) {
                    ...
                    try {
                        ...
                        if (res.status === 429) {
                            if (i === retries - 1)
                                throw new Error("Rate limited permanently");
            ...
            const ix1Req = await fetchWithTimeout('https://lite-api.jup.ag/swap/v1/swap-instructions', {
                ...
                body: JSON.stringify({ quoteResponse: quote1, userPublicKey: config_1.config.WALLET_PUBLIC_KEY, wrapAndUnwrapSol: true, prioritizationFeeLamports: "auto" })
            });
            const ix1 = await ix1Req?.json();
            const ix2Req = await fetchWithTimeout('https://lite-api.jup.ag/swap/v1/swap-instructions', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'x-api-key': API_KEY },
                body: JSON.stringify({ quoteResponse: quote2, userPublicKey: config_1.config.WALLET_PUBLIC_KEY, wrapAndUnwrapSol: true, prioritizationFeeLamports: "auto" })
            });
            const ix2 = await ix2Req?.json();
            const tipLamports = Math.floor(opp.tipAmount * 1e9);
            const transaction = await (0, transaction_1.buildVersionedTransaction)(ix1, ix2, tipLamports);
            if (transaction) {
                await (0, racing_1.submitTransactionWithRacing)(transaction);
            }
        }
        catch (e) {
            logger_1.logger.error(`Execution failed actively: ${e.message}`);
        }
    }
}
exports.ArbEngine = ArbEngine;
exports.globalArbEngine = new ArbEngine();
