...
async function forceTestTrade() {
    logger_1.logger.info("⚡ FORCING SINGLE BUY TEST (BONK) ⚡");
    ...
    logger_1.logger.info(`Quote received! Expected Output: ${(Number(quoteResponse.outAmount) / 10 ** 6).toFixed(4)} WIF`);
    ...
    logger_1.logger.info("3) Signing Transaction and Injecting Priority Fees...");
    const swapTransactionBuf = Buffer.from(swapTransaction, 'base64');
    let transaction = web3_js_1.VersionedTransaction.deserialize(swapTransactionBuf);
    transaction.sign([wallet]);
    logger_1.logger.info("4) Submitting via Target RPC with High Priority Allocation...");
    try {
