...
function main() {
  ...
  if (DRY_RUN) {
    ...
    console.log(`\n[TUNE] ✅ strategy_params.json updated`);

    // [KELLY BRIDGE] Publish Kelly-computed trade size to config:update so sniper uses it live
    const kellyConfig: Record<string, number> = {};
    if (newParams.MAX_TRADE_SIZE_SOL) kellyConfig.BUY_AMOUNT_SOL = newParams.MAX_TRADE_SIZE_SOL;
    if (newParams.MAX_SLIPPAGE_BPS)   kellyConfig.MAX_SLIPPAGE_BPS = newParams.MAX_SLIPPAGE_BPS;
    if (Object.keys(kellyConfig).length > 0) {
      RedisBus.publish('config:update', kellyConfig)
        .then(() => console.log(`[TUNE] 📡 Kelly config pushed to sniper: BUY_AMOUNT_SOL=${kellyConfig.BUY_AMOUNT_SOL}`))
        .catch((e: any) => console.error(`[TUNE] ❌ Redis publish failed:`, e.message));
    }
  }
}

// Redis Pub/Sub Daemon — DECOUPLED from config:update to prevent feedback loops
function startOptimizerDaemon() {
  console.log(`[OPTIMIZER] 🛠️ Initializing Kelly Optimizer Daemon...`);
  const subscriber = RedisBus.getSubscriber();

  // Listen on dedicated critic channel (NOT config:update, which WE publish to)
  subscriber.subscribe('critic:proposals', (err: any, count: any) => {
    if (err) console.error(`[OPTIMIZER] ❌ Redis Subscribe Error:`, err);
    else console.log(`[OPTIMIZER] ✅ Listening for Critic proposals on critic:proposals...`);
  });

  subscriber.on('message', (channel: string, message: string) => {
    if (channel === 'critic:proposals') {
      console.log(`[OPTIMIZER] 📥 Critic proposal received — triggering Kelly recalibration...`);
      main();
    }
  });

  // Start initial tune on boot
  main();

  // Periodic recalibration every 5 minutes
  setInterval(() => {
    console.log(`[OPTIMIZER] ⏱️ Periodic recalibration...`);
    main();
  }, 5 * 60_000);
