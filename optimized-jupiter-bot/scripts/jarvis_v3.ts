...
const { Connection, Keypair, PublicKey, VersionedTransaction,
        ...
        ComputeBudgetProgram, AddressLookupTableAccount } = require('@solana/web3.js');
...
function circuitOk(symbol) {
  ...
  if (Date.now() > b.openUntil) { b.open = false; b.fails = 0; return true; } // HALF_OPEN
  return false;
}
...
function recordFailure(symbol) {
  const b = breakers[symbol];
  b.fails++;
  if (b.fails >= 4) { b.open = true; b.openUntil = Date.now() + 30000; } // 30s cooldown
}
...
setInterval(refreshJitoTips, 3600_000);
...
async function buildTx(ix1, ix2, tipLamports) {
  try {
    ...
    valid.unshift(ComputeBudgetProgram.setComputeUnitLimit({ units: 1_400_000 }));
...
async function scanToken(symbol, mint, tradeLamports) {
  ...
  const q1Res = await safeFetch(
    `${JUP_BASE}/quote?inputMint=${SOL_MINT}&outputMint=${mint}&amount=${tradeLamports}&slippageBps=${SLIPPAGE}&asLegacyTransaction=false&excludeDexes=${EXCLUDE_DEXES}`
  );
  const q1 = await q1Res.json();
  if (!q1.outAmount) throw new Error(`Quote1 missing outAmount: ${JSON.stringify(q1).slice(0,80)}`);

  const q2Res = await safeFetch(
    `${JUP_BASE}/quote?inputMint=${mint}&outputMint=${SOL_MINT}&amount=${q1.outAmount}&slippageBps=${SLIPPAGE}&asLegacyTransaction=false&excludeDexes=${EXCLUDE_DEXES}`
  );
  const q2 = await q2Res.json();
  if (!q2.outAmount) throw new Error(`Quote2 missing outAmount`);
...
function printDashboard() {
  ...
  const topTokens = Object.entries(stats.perToken)
    .filter(([, v]) => (v as any).trades > 0)
    .sort(([, a], [, b]) => (b as any).pnl - (a as any).pnl)
    .slice(0, 5);
  if (topTokens.length) {
    console.log('  Top Performers:');
    topTokens.forEach(([sym, v]: [string, any]) => {
...
async function main() {
  ...
  while (true) {
    ...
    if (cycle % 20 === 0) {
      try { liveBalance = (await connection.getBalance(wallet.publicKey)) / 1e9; } catch(_) {}
    }
    ...
    const scanPromises = VALID_TARGETS.map(async t => {
      try {
        ...
        if (!e.message.includes('RATE_LIMITED') && !e.message.includes('is not tradable')) {
          ...
          if (cycle % 50 === 0) process.stdout.write(`[${t.symbol}!]`);
        }
    ...
    if (opportunities.length === 0) {
      ...
      try {
        ...
        if (!tx) throw new Error('Transaction builder returned null');
