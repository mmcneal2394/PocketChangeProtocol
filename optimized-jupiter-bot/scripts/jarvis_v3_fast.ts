...
const { Connection, Keypair, PublicKey, VersionedTransaction,
        TransactionMessage, TransactionInstruction, SystemProgram,
...
const EXCLUDE_DEXES = encodeURIComponent(
...
async function fetchBlockhashFromRpc(endpoint: string): Promise<string | null> {
  try {
...
function getBlockhash(): string {
  if (!cachedBlockhash) throw new Error('Blockhash not ready');
  const staleMs = Date.now() - blockhashAge;
  if (staleMs > 60000) throw new Error(`Blockhash stale (${staleMs}ms)`);
...
async function refreshTipAccounts() {
  try {
    ...
    if (d?.result?.length > 0) jitoTipAccounts = d.result;
  } catch(_) {}
}
setInterval(refreshTipAccounts, 3_600_000);
refreshTipAccounts();

// ─── ALT cache ────────────────────────────────────────────────────────────────
const altCache: Record<string, any> = {};
async function getALT(addr: string) {
  if (altCache[addr]) return altCache[addr];
  try {
    const acct = await heliusConn.getAddressLookupTable(new PublicKey(addr));
    if (acct?.value) altCache[addr] = acct.value;
  } catch(_) {}
...
async function buildTx(ix1: any, ix2: any, tipLamports: number) {
  try {
    ...
    const deser = (ix: any) => {
      ...
      return new TransactionInstruction({
        ...
        keys: ix.accounts.map((k: any) => ({ pubkey: new PublicKey(k.pubkey), isSigner: k.isSigner, isWritable: k.isWritable })),
        data: Buffer.from(ix.data, 'base64')
      });
    };

    const altsToFetch = [...(ix1.addressLookupTableAddresses || []), ...(ix2.addressLookupTableAddresses || [])];
    const alts = (await Promise.all([...new Set(altsToFetch)].map((a: any) => getALT(a)))).filter(Boolean);

    (ix1.setupInstructions || []).forEach((ix: any) => instructions.push(deser(ix)));
    instructions.push(deser(ix1.swapInstruction));
    if (ix1.cleanupInstruction) instructions.push(deser(ix1.cleanupInstruction));
    (ix2.setupInstructions || []).forEach((ix: any) => instructions.push(deser(ix)));
    instructions.push(deser(ix2.swapInstruction));
    if (ix2.cleanupInstruction) instructions.push(deser(ix2.cleanupInstruction));
    ...
    valid.unshift(ComputeBudgetProgram.setComputeUnitLimit({ units: 300_000 }));    // ← Audit shows avg 204k CU used, cap at 300k
...
// ═════════════════════════════════════════════════════════════════════════════
async function broadcastBundle(tx: any): Promise<{ result: string | null; error: string | null; ms: number; source: string }> {
...
// ─── Circuit Breaker ──────────────────────────────────────────────────────────
const breakers: Record<string, { fails: number; open: boolean; openUntil: number }> = {};
TARGETS.forEach(t => { breakers[t.symbol] = { fails: 0, open: false, openUntil: 0 }; });
const circuitOk   = (s: string) => { const b = breakers[s]; if (!b.open) return true; if (Date.now() > b.openUntil) { b.open = false; b.fails = 0; return true; } return false; };
const recSuccess  = (s: string) => { breakers[s].fails = 0; breakers[s].open = false; };
const recFailure  = (s: string) => { const b = breakers[s]; b.fails++; if (b.fails >= 5) { b.open = true; b.openUntil = Date.now() + 30000; } };

// ─── PnL Tracker ─────────────────────────────────────────────────────────────
const stats = {
  startTime: Date.now(), startBalance: 0, scans: 0, trades: 0, wins: 0, totalPnl: 0,
  perToken: {} as Record<string, { scans: number; trades: number; pnl: number }>
};
...
setInterval(printDashboard, 30_000);

// ─── Scan token ───────────────────────────────────────────────────────────────
async function scanToken(symbol: string, mint: string, tradeLamports: number) {
  if (!circuitOk(symbol)) return null;
  stats.scans++;
  ...
  if (!q1.outAmount) throw new Error('Quote1 no outAmount');
  ...
  if (!q2.outAmount) throw new Error('Quote2 no outAmount');

  const gross  = Number(q2.outAmount) / 1e9 - tradeLamports / 1e9;
  const tipLam = Math.max(100_000, Math.min(Math.floor(gross * 1e9 * 0.5), 5_000_000));
...
async function main() {
  ...
  let waited = 0;
  while (!cachedBlockhash && waited < 3000) { await new Promise(r => setTimeout(r, 100)); waited += 100; }
  if (!cachedBlockhash) throw new Error('Blockhash never arrived from either RPC');
  ...
  while (true) {
    ...
    if (cycle % 15 === 0) {
      try { liveBalance = (await nextConn().getBalance(wallet.publicKey)) / 1e9; } catch(_) {}
    }
    ...
    const results = await Promise.all(TARGETS.map(async t => {
      try { return await scanToken(t.symbol, t.mint, tradeLam); }
      catch (e: any) {
        if (!e.message.includes('RATE_LIMITED')) recFailure(t.symbol);
        return null;
      }
    }));
    ...
    try {
      ...
      if (ix1.error || ix2.error) throw new Error(`swap-instructions error: ${ix1.error || ix2.error}`);

      const tx = await buildTx(ix1, ix2, best.tipLam);
      if (!tx) throw new Error('buildTx returned null');
