...
async function getBlockhashRace(): Promise<{ blockhash: string; source: string; ms: number }> {
...
async function jupFetch(path: string, opts: any = {}) {
...
async function executeUltraLeg(inputMint: string, outputMint: string, amount: number) {
  const t0    = Date.now();
  ...
  if (order.error) throw new Error(`Order: ${JSON.stringify(order).slice(0,100)}`);

  const tSign0 = Date.now();
  const buf    = Buffer.from(order.transaction, 'base64');
  let tx: any;
  try { tx = VersionedTransaction.deserialize(buf); tx.sign([wallet]); }
  catch(_) { tx = Transaction.from(buf); tx.sign(wallet); }
...
async function roundTrip(token: { symbol: string; mint: string }) {
  ...
  if (leg1.status !== 'Success') throw new Error(`Leg1 failed [${leg1.status}]: ${leg1.error}`);
  ...
  if (leg2.status !== 'Success') throw new Error(`Leg2 failed [${leg2.status}]: ${leg2.error}`);
