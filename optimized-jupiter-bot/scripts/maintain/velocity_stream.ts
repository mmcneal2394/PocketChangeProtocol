...
// ── Config ────────────────────────────────────────────────────────────────────
const RPC_HTTP   = process.env.RPC_ENDPOINT!;   // https://...chainstack.../TOKEN
const WS_URL     = RPC_HTTP.replace('https://', 'wss://').replace('http://', 'ws://');
const SIGNALS_DIR= path.join(process.cwd(), 'signals');
const VEL_FILE   = path.join(SIGNALS_DIR, 'velocity.json');
const WINDOW_MS  = 60_000;   // 60s rolling window
const WRITE_MS   = 2_000;    // write every 2s
...
const fetchQueue   = new Set<string>(); // signatures pending fetch
...
async function fetchAndParseMint(signature: string, isBuy: boolean) {
  if (processedSig.has(signature) || fetchQueue.has(signature)) return;
  if (activeFetches >= MAX_CONCURRENT_FETCHES) { fetchQueue.add(signature); return; }

  fetchQueue.delete(signature);
  ...
  try {
    ...
    activeFetches--;
    // Drain queue
    const next = fetchQueue.values().next().value;
    if (next) fetchAndParseMint(next, true);
  }
}
...
function parsePumpLog(logs: string[], signature: string): void {
  ...
  // Try to extract mint directly from log lines (Pump.fun logs mint in some versions)
  // Look for 43-44-char base58 strings in log output — skip known program/system addresses
  const mintRegex = /\b([1-9A-HJ-NP-Za-km-z]{43,44})\b/g;
  // All base currencies and system addresses — NOT token mints
  const SKIP_ADDRS = new Set([
    ...
    '7vfCXTUXx5WJV5JADk17DUJ4ksgau7utNKj4b963voxs',  // ETH (Wormhole)
    'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA',   // Token program
    'ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJe8bv',   // ATA program  ← was leaking into hot mints
    'ComputeBudget111111111111111111111111111111',     // Compute budget
    '6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P',   // Pump.fun prog
    'BSfD6SHZigAfDWSjzD5Q41jw8LmKwtmjskPH9XW1mrRW',  // Pump.fun bonding
    '675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8',  // Raydium V4
    'CAMMCzo5YL8w4VFF8KVHrK22GGUsp5VTaW7grrKgrWqK',  // Raydium CLMM
    'SysvarRent111111111111111111111111111111111',     // Sysvar Rent
    'SysvarC1ock11111111111111111111111111111111',     // Sysvar Clock
    '11111111111111111111111111111111',                // System program
    'JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbPwdrsxGBK',    // Jupiter
  ...
  for (const log of logs) {
    ...
    while ((match = mintRegex.exec(log)) !== null) {
      ...
      if (!SKIP_ADDRS.has(candidate) && candidate.length >= 43) {
        recordSwap(candidate, isBuy, 0);
        return; // got the mint from logs directly — no RPC call needed
      }
    }
  }

  // If no mint found in logs, fall back to signature fetch
...
function computeAndWrite() {
  ...
  fs.writeFileSync(VEL_FILE, JSON.stringify({ updatedAt: now, mintCount: hotCount, mints: out }, null, 2));
}
...
console.log('║  Real-time swap detection → signals/velocity.json║');
console.log('╚══════════════════════════════════════════════════╝');
