...
You are acting exactly on the core objective: MAXIMIZE NET WALLET ACCUMULATION OVER TIME.

You will receive:
1. Current Bot Wallet State (Net USD, SOL balances, Open Positions)
2. Performance findings from the bot's live trade journal (structured JSON)
3. Current bot strategy parameters

Your job: Given the current Wallet Balance, propose exactly 3 concrete, actionable parameter changes to accelerate capital accumulation and protect against drawdowns.
...
def run() -> list[dict]:
    ...
    # Fetch Latest Wallet State from Redis
    wallet_state = {}
    recent_trades = []
    try:
        r = redis.Redis(host='127.0.0.1', port=6379, decode_responses=True)
        raw_w = r.get("wallet:latest")
        if raw_w:
            wallet_state = json.loads(raw_w)
            
        # [V3 Streaming] Ingest last 50 raw execution trades natively from memory
        raw_trades = r.xrevrange("stream:trades", max="+", min="-", count=50)
        for t_id, t_data in raw_trades:
            recent_trades.append(t_data)
    except Exception as e:
        print(f"[CriticAgent] Redis fetch err: {e}")

    # Try LLM first
    if GEMINI_API_KEY and findings:
        user_prompt = (
            f"Wallet State (Prioritize Accumulating This!):\n{json.dumps(wallet_state, indent=2)}\n\n"
            f"Analytics Findings:\n{json.dumps(findings, indent=2)}\n\n"
            f"Recent Native Trades Stream (Context):\n{json.dumps(recent_trades, indent=2)}\n\n"
            f"Current params:\n{json.dumps(params, indent=2)}\n\n"
            "Propose 3 ranked improvements strictly focused on maximizing Wallet Growth."
        )
        llm_resp = call_gemini(SYSTEM_PROMPT + "\n\n" + user_prompt)
        if llm_resp:
            proposals = parse_llm_proposals(llm_resp)
            print(f"[CriticAgent] LLM generated {len(proposals)} proposal(s) targeting Net Value Accumulation")
    ...
    # [V2 Redis Integration] Push the proposals over the local localhost message bus
    try:
        r = redis.Redis(host='127.0.0.1', port=6379, decode_responses=True)
        r.publish("optimizer:update", json.dumps(output))
        print("[CriticAgent] 📡 Published AI proposals to Redis ('optimizer:update')")
    except Exception as e:
        print(f"[CriticAgent] ⚠️ Redis publish failed, using strictly fallback files {e}")
        
    # Heartbeat
    try:
        r = redis.Redis(host='127.0.0.1', port=6379, decode_responses=True)
        r.publish("heartbeat:agent", json.dumps({"agent": "pcp-critic", "timestamp": datetime.now(timezone.utc).isoformat()}))
    except Exception:
        pass
