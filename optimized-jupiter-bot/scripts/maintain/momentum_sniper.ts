...
const MIN_BUY_SOL      = parseFloat(process.env.SNIPER_MIN_BUY  || '0.005'); // floor
const MAX_BUY_SOL      = parseFloat(process.env.SNIPER_MAX_BUY  || '0.03');  // tighter cap
...
const MIN_VOLUME_1H    = parseFloat(process.env.SNIPER_MIN_VOL || '8000');  // $8k min 1h vol (raised from $5k)
const MIN_PRICE_CHG_1H = parseFloat(process.env.SNIPER_MIN_CHG || '3');    // +3% min 1h change
const MIN_BUY_RATIO    = parseFloat(process.env.SNIPER_MIN_BR  || '3.5');  // 3.5x buys vs sells (was 2.5x)
const MIN_BUYS_1H      = parseInt(process.env.SNIPER_MIN_BUYS  || '30');   // at least 30 buy txns (was 20)
const MAX_TOKEN_AGE_MIN= parseFloat(process.env.SNIPER_MAX_AGE  || '9999'); // no age limit — watch all tokens
const MIN_MOMENTUM_5M  = parseFloat(process.env.SNIPER_MIN_5M   || '2');   // require +2% 5-min momentum (sub-candle freshness)
const POLL_MS          = 20_000; // check every 20s
...
async function calcBuySize(): Promise<number> {
  try {
    // Primary: read WSOL token balance (persistent ATA — no wrap fee)
    const wsolBal = await getWsolBalance(connection, wallet.publicKey);
    // Fallback: if WSOL ATA empty, use native SOL balance
    const bal = wsolBal > 0 ? wsolBal : (await connection.getBalance(wallet.publicKey)) / 1e9;
    const raw      = bal * BASE_BUY_PCT;
    const weight   = loadSniperWeight();           // HarmonyAgent capital allocation
...
async function checkExits() {
  ...
  for (const pos of store.positions) {
    ...
    // Activates at +2% peak. Gets tighter as we go higher.
    ...
    else if (peak >= 2)  trailPct = peak * 0.40; // keep 60% — activate early
    else                 trailPct = 999;          // not activated yet
    const trail = peak >= 2 && pnlPct <= (peak - trailPct);

    if (tp || sl || trail || orderFlowReversal || forceExit) {
